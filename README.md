...
Or click [here](./instructions/7-cli.md) for instructions using the ASK CLI (command line interface). The ASK CLI (command line interface) cannot complete this task below and provides only partial completion. Therefore it is better to follow the instructions bewlow, by pressing the ** Getting Started **.
